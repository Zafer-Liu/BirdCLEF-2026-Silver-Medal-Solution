...
if 'Model_7' in _ensemble_models:
    ...
    if 'Karnakbayev_PowerOptimization_LB0948' in _ensemble_models:
        ...
        probe_models, emb_scaler, emb_pca, alpha_blend = train_mlp_probes(
            emb=emb_tr, scores_raw=sc_tr, Y=Y_FULL_aligned, min_pos=3, pca_dim=64, alpha_blend=0.4)
        ...
        _CORRECTION_GRID = [0.00, 0.04, 0.07, 0.10, 0.12, 0.15, 0.18, 0.20, 0.22, 0.25, 0.28, 0.30, 0.33, 0.35, 0.38, 0.40, 0.44, 0.48, 0.52]
        ...
        for i, path in enumerate(test_paths, 1):
            ...
            for sess in sed_sessions:
                ...
                ss = outs[1].max(axis=1)

                p_sum += sigmoid_sed(ss)
        ...
        # === v221 TAXONOMY SMOOTHING (post-processing) ===
        _alpha_genus = 0.15
        _alpha_class = 0.05

        _cols = list(sub.columns)
        if "row_id" in _cols:
            _cols.remove("row_id")
            
        probs_mat = sub[_cols].values.astype(np.float32)

        _multi_genus = {}
        _multi_class = {}

        # Reconstruct mappings from the loaded taxonomy dataframe
        for _, row in taxonomy.iterrows():
            if row['primary_label'] in _cols:
                genus = str(row['scientific_name']).split()[0]
                cls = str(row['class_name'])
                _multi_genus.setdefault(genus, []).append(row['primary_label'])
                _multi_class.setdefault(cls, []).append(row['primary_label'])

        _multi_genus = {k: v for k, v in _multi_genus.items() if len(v) > 1}
        _multi_class = {k: v for k, v in _multi_class.items() if len(v) > 1}

        # 1. Genus pass
        for _g, _members in _multi_genus.items():
            _idx = [_cols.index(_m) for _m in _members]
            _genus_mean = probs_mat[:, _idx].mean(axis=1, keepdims=True)
            probs_mat[:, _idx] = (1 - _alpha_genus) * probs_mat[:, _idx] + _alpha_genus * _genus_mean

        # 2. Class pass
        for _cl, _members in _multi_class.items():
            _idx = [_cols.index(_m) for _m in _members]
            _class_mean = probs_mat[:, _idx].mean(axis=1, keepdims=True)
            probs_mat[:, _idx] = (1 - _alpha_class) * probs_mat[:, _idx] + _alpha_class * _class_mean

        sub[_cols] = probs_mat
        print(f"[v221] Post-tax-smoothing: mean={sub[_cols].values.mean():.4f}")
